    ...
    if not get_id_tg_list_local_db:
        ...
        if arrays_equal is True:
            ...
            if len(not_values_in_jarvis_db) == 0:
                ...
                # подготовка даных и передать в функцию записи.
                for id_tg in not_values_in_jarvis_db:
                    id = int(id_tg)  # в нупи int64 - будет ошибка !!!
                    #
                    # Обновляем значение в строке на удаленный:
                    await update_delet_local_db(id, session_pool)

                # todo апдейт записей  удален ( !!! Не тестировалось. Тест пройдет, когда будут уволенные)
            ...
            if len(not_values_in_local_db) == 0:
                ...
                for id_tg in not_values_in_local_db:
                    id = int(id_tg) # в нупи int64 - будет ошибка !!!
                    int_id_tg_list.append(id)  #  передаем сразу весь список

                    # Преобразуем список в строку, пригодную для SQL-запроса
